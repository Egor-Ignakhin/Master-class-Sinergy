...
namespace Unity.FPS.AI
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        [HideInInspector]  public float SelfDestructYHeight = -20f;

        [Tooltip("The distance at which the enemy considers that it has reached its current path destination point")]
        [HideInInspector]public float PathReachingRadius = 2f;

        [Tooltip("The speed at which the enemy rotates")]
        [HideInInspector] public float OrientationSpeed = 10f;

        [Tooltip("Delay after death where the GameObject is destroyed (to allow for animation)")]
        [HideInInspector]public float DeathDuration = 0f;


        [Header("Weapons Parameters")] [Tooltip("Allow weapon swapping for this enemy")]
        [HideInInspector] public bool SwapToNextWeapon = false;

        [Tooltip("Time delay between a weapon swap and the next attack")]
        [HideInInspector]    public float DelayAfterWeaponSwap = 0f;
        ...
        [HideInInspector] public Color DefaultEyeColor;

        [Tooltip("The attack color of the bot's eye")] [ColorUsageAttribute(true, true)]
        [HideInInspector]  public Color AttackEyeColor;
        ...
        [HideInInspector] public Gradient OnHitBodyGradient;
        ...
        [HideInInspector]   public Color PathReachingRangeColor = Color.yellow;

        [Tooltip("Color of the sphere gizmo representing the attack range")]
        [HideInInspector]  public Color AttackRangeColor = Color.red;

        [Tooltip("Color of the sphere gizmo representing the detection range")]
        [HideInInspector]  public Color DetectionRangeColor = Color.blue;
        ...
        bool IsPathValid()
        {
            return PatrolPath && PatrolPath.Points.Count > 0;
        }
        ...
        public void UpdatePathDestination(bool inverseOrder = false)
        {
            if (IsPathValid())
            {
                ...
                if ((transform.position - GetDestinationOnPath()).magnitude <= PathReachingRadius)
                {
                    ...
                    {
                        m_PathDestinationNodeIndex += PatrolPath.Points.Count;
                    }

                    if (m_PathDestinationNodeIndex >= PatrolPath.Points.Count)
                    {
                        m_PathDestinationNodeIndex -= PatrolPath.Points.Count;
                    }
                }
